...
Editor.prototype = {
    constructor: Editor,
    changeContainer: function changeContainer(dom) {
        var _this = this;

        // 编辑区域
        var $textElem = $('<div></div>');
        var $textContainerElem = $(dom);
        var config$$1 = this.config;
        var zIndex = config$$1.zIndex;
        var $hasTextElem = $textContainerElem.find('.w-e-text');
        if ($hasTextElem.length > 0) {
            $textElem = $($hasTextElem[0]);
        } else {
            $textElem.attr('contenteditable', 'true').css('width', '100%').css('height', '100%');
            var $children = $textContainerElem.children();
            // 初始化编辑区域内容
    ...
    _initDom: function _initDom() {
        ...
        $textContainerElem.on('click keyup', function () {
            // 输入法结束才出发 onchange
            compositionEnd && _this2.change && _this2.change();
        });
